...
| Email đăng ký mới customer | customer.manual.test+01@example.com | Dùng để test đăng ký mới |
| Email đăng ký mới driver | driver.manual.test+01@example.com | Dùng để test đăng ký mới |
...
- Driver App hiện chưa có luồng chỉnh sửa hồ sơ tài xế trên giao diện.
- Driver App trước đây không hiển thị rõ trạng thái chờ duyệt; cần kiểm tra lại sau bản sửa này.
- Admin Dashboard hiện chưa có thao tác duyệt hoặc từ chối hồ sơ tài xế theo đúng nghiệp vụ phê duyệt.
- Backend driver-service có trạng thái `PENDING | APPROVED | REJECTED | SUSPENDED`, nhưng route admin đang lộ ra là `verify`, chưa phải một luồng approve hoàn chỉnh.
- Giá trị rating mặc định từ backend có thể làm tài khoản tài xế mới trông như đã có 5/5 nếu UI không xử lý trường hợp chưa có review.

### 4.2. Các vấn đề nghi ngờ cần kiểm tra thực tế

- GPS hiện tại có thể lệch so với vị trí thật của thiết bị hoặc trình duyệt.
- Cụm nhập điểm đón và điểm đến có thể bị lớp bản đồ hoặc overlay làm mờ, khó thao tác.
- Luồng đặt xe có thể bị loading kéo dài hoặc kẹt ở bước tìm tài xế.
...
| --- | --- | --- | --- |
| 1 | Bỏ trống toàn bộ trường | Kiểm tra ràng buộc bắt buộc | Không cho submit, hiển thị lỗi yêu cầu nhập đủ trường |
| 2 | Email sai định dạng, ví dụ `abc` | Kiểm tra validate email | Hiển thị lỗi email không hợp lệ |
| 3 | Mật khẩu dưới 8 ký tự | Kiểm tra độ dài tối thiểu | Hiển thị lỗi độ dài mật khẩu |
| 4 | Mật khẩu không có chữ hoa | Kiểm tra rule chữ hoa | Hiển thị lỗi tương ứng |
| 5 | Mật khẩu không có chữ thường | Kiểm tra rule chữ thường | Hiển thị lỗi tương ứng |
| 6 | Mật khẩu không có số | Kiểm tra rule chữ số | Hiển thị lỗi tương ứng |
| 7 | Mật khẩu và xác nhận mật khẩu khác nhau | Kiểm tra so khớp mật khẩu | Hiển thị lỗi không khớp |
| 8 | Số điện thoại chứa chữ hoặc quá ngắn | Kiểm tra validate phone | Hiển thị lỗi số điện thoại |
| 9 | Dữ liệu hợp lệ | Kiểm tra luồng đăng ký thành công | Chuyển về màn hình đăng nhập hoặc báo đăng ký thành công |

### B. Kiểm tra đăng nhập khách hàng

| STT | Dữ liệu nhập | Mục đích kiểm tra | Kết quả mong đợi |
| --- | --- | --- | --- |
| 1 | Bỏ trống email và mật khẩu | Kiểm tra chặn submit phía client | Hiển thị lỗi yêu cầu nhập đủ trường |
| 2 | Email sai định dạng | Kiểm tra validate cơ bản phía client | Hiển thị lỗi email không hợp lệ |
| 3 | Mật khẩu ngắn hơn 8 ký tự | Kiểm tra điều kiện đầu vào | Hiển thị lỗi mật khẩu chưa hợp lệ |
| 4 | Email hoặc mật khẩu sai | Kiểm tra xử lý lỗi backend | Hiển thị thông báo đăng nhập thất bại |
| 5 | Tài khoản hợp lệ | Kiểm tra luồng đăng nhập thành công | Vào trang chủ customer |
...
Thực hiện tương tự customer cho các trường email, mật khẩu, xác nhận mật khẩu và số điện thoại, sau đó kiểm tra thêm luồng `Profile Setup`.

| STT | Dữ liệu nhập | Mục đích kiểm tra | Kết quả mong đợi |
| --- | --- | --- | --- |
| 1 | Dữ liệu auth không hợp lệ | Kiểm tra validate đăng ký | Không cho submit, báo lỗi rõ ràng |
| 2 | Dữ liệu auth hợp lệ | Kiểm tra tạo tài khoản driver | Điều hướng sang màn hình hoàn tất hồ sơ |
...
| --- | --- | --- | --- |
| 1 | Bỏ trống email và mật khẩu | Kiểm tra chặn submit phía client | Hiển thị lỗi yêu cầu nhập đủ trường |
| 2 | Email sai định dạng | Kiểm tra validate email | Hiển thị lỗi email không hợp lệ |
| 3 | Mật khẩu ngắn hơn 8 ký tự | Kiểm tra điều kiện đầu vào | Hiển thị lỗi mật khẩu chưa hợp lệ |
| 4 | Tài khoản sai | Kiểm tra lỗi backend | Hiển thị thông báo đăng nhập thất bại |
| 5 | Tài khoản hợp lệ | Kiểm tra điều hướng | Vào dashboard tài xế |
...
| 2 | Quan sát app bar hoặc profile | Kiểm tra user có nhìn thấy đang chờ duyệt hay không | Có chip hoặc nhãn thể hiện `Chờ duyệt hồ sơ` |
...
| 5 | Kiểm tra khả năng sửa hồ sơ | Xác nhận gap chức năng | Hiện chưa có màn hình sửa hồ sơ, ghi nhận là thiếu tính năng |
...
| Bước | Thao tác | Mục đích | Kết quả mong đợi |
| --- | --- | --- | --- |
| 1 | Mở trang Drivers | Kiểm tra có danh sách tài xế chờ duyệt không | Có dữ liệu driver |
| 2 | Tìm thao tác duyệt hoặc từ chối | Kiểm tra khả năng xử lý nghiệp vụ | Hiện tại dashboard chưa có nút duyệt hoặc từ chối hoàn chỉnh, cần ghi nhận là thiếu tính năng |
...
- Thực hiện: chạy `npm run db:seed`.
- Mục đích: tạo lại tài khoản test cố định.
- Kết quả mong đợi: các tài khoản seed đăng nhập được.
...
- Nếu giao diện customer bị che nút bởi thanh điều hướng dưới, cuộn hoặc thu nhỏ cửa sổ để thao tác rõ hơn.
