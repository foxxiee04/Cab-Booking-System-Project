...
File này được viết theo hướng có thể sao chép sang Microsoft Word và định dạng lại theo mẫu khóa luận. Khi đưa sang Word, nên giữ cấu trúc chương/mục hiện có, sau đó chèn hình từ thư mục **`img/`** (gốc repo, cùng cấp với `docs/`) theo bảng gợi ý dưới đây — **không tạo thêm thư mục ảnh mới**; nếu cần hình bổ sung, xuất PNG và đặt tên theo quy ước `img/NN_ten_mo_ta.png` rồi cập nhật bảng này. Các đoạn `![Hình ...](../img/...)` trong file là vị trí gợi ý chèn ảnh; nếu Word không tự nhận Markdown image, dùng **Insert > Pictures** và chọn đúng file trong `img/`.
...
| Chương 6 hoặc Hướng phát triển | `img/02_aws_deployment_architecture.png` | Kiến trúc triển khai cloud đề xuất (tham chiếu) |
| Chương 6, triển khai Swarm thực tế | `img/21_aws_swarm_deployment_actual.png` | Luồng DNS → Docker Swarm AWS, CI/CD SSH (theo `deploy/SWARM-SETUP.md`) |
| Chương 6 hoặc 7, topology mục tiêu | `img/22_aws_target_reference_topology.png` | Topology/hạ tầng tham chiếu (nếu dùng trong báo cáo) |
| Chương 6, CI/CD | `img/23_cicd_pipeline_github_actions.png` | Pipeline GitHub Actions → Docker Hub → deploy |

### Ánh xạ 12 phần nội dung (theo đề cương giảng viên) → chương/mục trong file này

Cấu trúc **Chương 1–8** của báo cáo giữ nguyên. Mười hai phần dưới đây **lồng ghép** bằng cách: (i) mỗi phần trỏ tới **đúng mục đã có**; (ii) các mục **4.0, 4.12–4.13, 5.6–5.8, 6.8–6.9, 7.4** bổ sung cho đủ checklist phân tích/triển khai. Khi dàn Word, có thể đặt tiêu đề «Phần 1… Phần 12» trong mục lục hoặc chỉ dùng bảng này làm «mục lục song song».
...
| **7** Ứng dụng AI / pipeline | Surge, matching hỗ trợ, RAG chatbot | **§5.8**; chi tiết **§4.11.12**, **§3.6.9**; hình `img/06_*`, `img/16_*` |
| **8** Giám sát hệ thống | Prometheus, Grafana, node metrics | **§6.8 Chương 6** |
| **9** Bảo mật thông tin, dữ liệu | Trust boundary, JWT, IPN, nội bộ | **§6.7.1**; hình `img/20_*` |
| **10** Cài đặt và triển khai (CI/CD) | GitHub Actions, Docker, Swarm | **§6.9**; hình `img/23_*`, `img/21_*` |
| **11** Tối ưu mở rộng (scalability) | Stateless, scale service, Swarm | **§7.4 Chương 7** |
...
| 3.10 | **Data ownership** (database per service) | **§4.10**; hình `img/05_data_architecture.png` |
...
| **Domain Event + Observer** | Publish–subscribe qua RabbitMQ, giảm coupling BC | **§4.5**, `img/13_event_flow_rabbitmq.png` |
...
| **Adapter** | Redis Adapter cho Socket.IO cluster; HTTP↔gRPC bridge | **§6.2**, `img/19_component_api_gateway.png` |
...
**Hình gợi ý (thư mục `img/`):** `img/06_ai_ml_pipeline.png`, `img/16_rag_chatbot_architecture.png`.
...
**Tham chiếu kiến trúc đám mây “đích” (ECS/RDS/…):** có thể trình bày song song để so sánh với Swarm thực tế — hình **`img/02_aws_deployment_architecture.png`**.

**Hình gợi ý chèn vào Word:** `img/02_aws_deployment_architecture.png`

![Hình 6.2. Kiến trúc triển khai AWS tham chiếu (ECS/RDS/...)](../img/02_aws_deployment_architecture.png)
...
Trong báo cáo Word, có thể chèn **`img/21_aws_swarm_deployment_actual.png`** để minh họa **luồng CI/CD + DNS → Swarm** (phần observability nằm cùng cluster).
...
**Hình gợi ý (thư mục `img/`):** `img/23_cicd_pipeline_github_actions.png`.

> **Phần 9 — Bảo mật:** đã trình bày tại **§6.7.1** và hình `img/20_security_trust_boundary.png`.
