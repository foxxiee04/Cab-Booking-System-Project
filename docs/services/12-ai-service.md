...
| Optional LLM | OpenAI GPT, Gemini fallback |
...
1. Startup:
   ...
   Chunk Q&A-aware, giữ Hỏi/Đáp cùng ngữ cảnh
   Encode bằng Vietnamese bi-encoder
   Build FAISS cosine + BM25 tiếng Việt
   Lazy-load cross-encoder reranker nếu bật

2. Request:
   User message → quick smalltalk nếu phù hợp
   Optional LLM query rewrite cho câu ngắn / follow-up
   Hybrid retrieval: FAISS + BM25 + RRF + precision pass
   Nếu reranker active: rerank top-N rồi lấy top_k
   Confidence gate bằng cosine: no_context / excerpt / LLM synthesis
   LLM priority: OpenAI GPT → Gemini → rulebase/template fallback

3. Response:
   { answer, sources, score_max, mode, latency_ms,
     llm_provider, llm_model, reranker_active, rewrite_used }
```

**Embedding model mặc định:** `bkai-foundation-models/vietnamese-bi-encoder`

**Reranker mặc định:** `BAAI/bge-reranker-v2-m3`. Tắt `RAG_RERANKER_ENABLED=false` sẽ nhẹ RAM hơn nhưng thường giảm độ chính xác; fallback hybrid vẫn chạy với cosine + keyword overlap để hạn chế lấy nhầm đoạn.
...
| `RAG_EMBEDDING_MODEL` | Model embedding (mặc định: Vietnamese bi-encoder) |
| `RAG_LLM_PROVIDER` | `none` \| `openai` \| `gemini` \| `groq` \| `claude` \| `auto`; `auto` thử OpenAI rồi Gemini |
| `OPENAI_API_KEY` | Key OpenAI GPT, provider ưu tiên |
| `GEMINI_API_KEY` | Key Gemini, fallback khi OpenAI lỗi/không có key |
| `GROQ_API_KEY` | Key Groq nếu chọn explicit `RAG_LLM_PROVIDER=groq` |
| `RAG_LLM_MODEL` | Model ID LLM |
| `RAG_RERANKER_ENABLED` | Bật/tắt reranker cross-encoder |
| `RAG_RERANK_POOL` | Số candidate hybrid đưa vào reranker / precision pass |
| `RAG_QUERY_REWRITE_ENABLED` | Bật/tắt rewrite truy vấn trước retrieval |
| `RAG_QUERY_REWRITE_PROVIDER` | `auto` ưu tiên Gemini, `same` dùng provider trả lời |
